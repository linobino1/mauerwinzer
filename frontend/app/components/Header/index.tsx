...
const Header: React.FC<Props> = ({ site, navigations }) => {
  ...
  const toggleModal = (slug: 'menu' | 'reservation' | 'closeAll') => {
    const newState = slug === 'closeAll' || slug === modal ? null : slug
    if (newState === null) {
      document.body.style.overflow = 'auto'
    } else {
  ...
  return (
    <>
      <header className={classes.mobile}>
        <Button className={classes.hamburger} onClick={() => toggleModal('menu')}>
      ...
      <div className={classes.modalHost}>
        ...
        <Modal show={modal === 'reservation'}>
          <header>
            <Button onClick={() => toggleModal('closeAll')} layout="symbol" symbol="close" />
          </header>
          <ReservationForm site={site} active={modal === 'reservation'} />
        </Modal>
      </div>
    </>
  )
